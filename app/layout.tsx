...
@media (max-width:900px){
  ...
  .intro{padding:16px 18px 34px;display:block;min-height:100svh}
  .topline{display:grid;grid-template-columns:auto 1fr;gap:14px;padding:0 0 14px;border-bottom:1px solid var(--line);font-size:9px}
  .topline .status{display:grid;grid-template-columns:auto 1fr;gap:7px 14px;justify-items:end}
  .topline .status > span:nth-child(2){grid-column:1/-1;grid-row:2;justify-self:start;white-space:normal;color:var(--red);font-size:8px;letter-spacing:.11em}
  .topline .status > span:last-child{grid-column:1/-1;grid-row:3;justify-self:start;display:flex;gap:18px;margin-top:2px}
  .intro a{margin-left:0}

  .intro-main{padding:70px 0 0}
  .kicker{font-size:8px;letter-spacing:.14em;margin-bottom:15px;white-space:nowrap}
  .hero-word{font-size:clamp(70px,23vw,150px);line-height:.78;letter-spacing:-.08em}
  .hero-word .ghost{transform:translateX:8vw;-webkit-text-stroke:.8px var(--ink)}
  .hero-copy{margin:38px 0 0 12vw;font-size:18px;line-height:1.08;letter-spacing:-.025em}

  .intro-aside{margin:64px 0 0;padding:17px 0 0;border-top:1px solid var(--line);font-size:10px;line-height:1.45}
  .intro-aside .big{font-size:31px;line-height:.95;margin-bottom:11px}
  .scroll-note{padding:0;margin-top:52px;display:flex;align-items:end;font-size:8px;letter-spacing:.12em;gap:20px}
  .scroll-note span:last-child{margin-left:auto;text-align:right}

  .section-head{padding:78px 18px 44px;display:block}
  .section-head h2{font-size:clamp(48px,15vw,88px);line-height:.83}
  .section-head p{margin-top:34px;max-width:470px;font-size:13px;line-height:1.55}

  .method-strip{grid-template-columns:1fr 1fr;padding:18px;gap:22px 18px;font-size:10px;line-height:1.25}
  .method-strip span{font-size:8px}

  .work-intro{display:block;padding:62px 18px 58px}
  .work-intro>div:last-child{padding-left:0;margin-top:58px}
  .work-intro h2{font-size:clamp(34px,10vw,58px);line-height:.94;margin-top:38px;max-width:580px}
  .work-intro>div:last-child span{font-size:clamp(64px,18vw,105px)}
  .work-intro p{font-size:14px;line-height:1.5;margin-top:42px;max-width:460px}

  .project{display:flex;flex-direction:column;min-height:0}
  .project-index{order:0;border-right:0;border-bottom:1px solid var(--line);padding:11px 18px}
  .project-index span{writing-mode:horizontal-tb;transform:none;font-size:8px;letter-spacing:.12em}
  .project-body{order:1;padding:42px 18px 48px;display:block}
  .project-type{font-size:8px;letter-spacing:.12em}
  .project-title{font-size:clamp(54px,16vw,92px);line-height:.82;letter-spacing:-.065em;margin:25px 0 21px;max-width:100%}
  .project-desc{font-size:15px;line-height:1.42;max-width:580px}
  .project-meta{grid-template-columns:1fr;gap:15px;margin-top:34px;padding-top:14px}
  .project-meta span{font-size:8px}
  .project-meta b{font-size:11px;margin-top:4px}
  .project-long{margin-top:29px;max-width:580px}
  .project-link{margin-top:25px;font-size:9px}
  .project-result{margin-top:42px;padding-top:18px;max-width:100%}
  .project-result strong{font-size:clamp(52px,16vw,84px)}
  .project-result span{font-size:9px;line-height:1.2}
  .project-result small{font-size:8px}

  .project-art{order:2;min-height:290px;height:290px}
  .signal{width:92%;height:70%}
  .art-label{top:14px;left:15px;font-size:8px}
  .art-coord{right:15px;bottom:14px;font-size:8px}

  .interlude{min-height:66svh;padding:92px 18px 76px}
  .interlude h3{font-size:clamp(55px,15vw,94px);line-height:.84;letter-spacing:-.06em}
  .interlude .tiny{right:18px;bottom:24px;font-size:8px;max-width:180px}

  .about{padding:88px 18px 105px;display:block}
  .about h2{font-size:clamp(58px,16vw,100px)}
  .about-copy{margin-top:45px;gap:23px}
  .about-copy p{font-size:16px;line-height:1.42}
  .about-facts{margin-top:34px}
  .about-facts>div{grid-template-columns:74px minmax(0,1fr);gap:15px;padding:17px 0}
  .about-facts strong{font-size:13px;line-height:1.4}
  .about-facts small{display:none}

  .contact{padding:24px 18px 30px;min-height:74svh}
  .contact-top{font-size:8px;line-height:1.35;gap:20px}
  .contact-top span:last-child{text-align:right;max-width:145px}
  .contact h2{font-size:clamp(72px,20vw,125px);line-height:.78;letter-spacing:-.08em}
  .contact-links{gap:17px;font-size:8px}
  .copyright{font-size:8px;margin-top:44px}
}
